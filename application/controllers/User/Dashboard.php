...
class dashboard extends CI_Controller {
	...
	public function __construct()
	{
		parent::__construct();
		// is_logged_in2();
		$this->load->model('dashboard_Model','dataproject');
	...
	public function index()
	{
		...
		$this->load->view('user/dashboard/vw_dashboard',$data);
	...
	function tambah()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->load->view("user/dashboard/vw_tambah", $data);
			$this->load->view("user/footer");
		}  else {
			$data = [
	...
	public function edit($id)
	{
		...
		if ($this->form_validation->run() == false) {
			$this->load->view("user/header", $data);
			$this->load->view("user/dashboard/vw_ubah", $data);
			$this->load->view("user/footer");
		} else {
			$data = [
				'name' => $this->input->post('name'),
